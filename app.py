...
"""
        with st.spinner("Building your itinerary..."):
            try:
                ...
                itinerary_text = (response.get("output") or "").strip()
                if not itinerary_text:
                    itinerary_text = (
                        "The assistant couldn't put together a full plan this "
                        "time (it may have gotten stuck resolving a tool call). "
                        "Try again, or check the terminal running `streamlit "
                        "run app.py` for the Thought/Action trace."
                    )
            except Exception as exc:
                itinerary_text = f"Something went wrong while generating the plan: {exc}"
                response = {"output": itinerary_text}
...
if question:
    with st.spinner("Thinking..."):
        try:
            ...
            if not answer:
                answer = (
                    "I couldn't work out a full answer that time - try "
                    "rephrasing with an explicit date (e.g. 'in September "
                    "2026' instead of 'this month')."
                )
        except Exception as exc:
